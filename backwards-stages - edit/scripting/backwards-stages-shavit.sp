...
native int TraceTriggers(int client, float flStart[3], float flEnd[3]);
#endif
...
ArrayList g_NormalPlayerTimesTriggerstoTrigger[MAXPLAYERS+1];
ArrayList g_BonusPlayerTimesTriggerstoTrigger[MAXPLAYERS+1];

ArrayList g_NormalPlayerTimesTriggerstoTriggerCurrentRunOnly[MAXPLAYERS+1];
ArrayList g_BonusPlayerTimesTriggerstoTriggerCurrentRunOnly[MAXPLAYERS+1];

int PreviousPlayerTrigger[MAXPLAYERS+1] = {3000, ...};
int g_PreviousIgnoreTrigger[MAXPLAYERS+1] = {3001, ...};
int g_TeleportDestionations[2048+1] = {-1, ...};
...
int g_PlayerCurrentStage[MAXPLAYERS+1] = {0, ...};

ArrayList PlayerRecordStringEntries[MAXPLAYERS+1];
char PlayersRecordsString[MAXPLAYERS+1][512];

int PlayersRecordsStringRGBforSpecs[MAXPLAYERS+1][3];
int PlayersRecordStringRGB[MAXPLAYERS+1][3];
float g_PlayerPastStageUpdateTime[MAXPLAYERS+1] = {0.0, ...};

Handle g_hStagesEnabled;

EngineVersion iEngineVersion = Engine_Unknown;
...
void ResetPlayerCurrentRun(int client)
{
	g_PlayerPastStageUpdateTime[client] = 0.0;
	g_PreviousIgnoreTrigger[client] = 3001;
	PreviousPlayerTrigger[client] = 3000;
...
public void FindAndMarkTeleportDestinations()
{
		...
		if(IsValidEntity(entity) && IsValidEdict(entity))
		{
			...
			if(StrEqual(className, "trigger_teleport", false))
			{
				...
				*/
				...
				char targetname[228], destination_name[228];
				...
				for(int destination_entity = MaxClients;destination_entity < GetMaxEntities();destination_entity++)
				{
...
int CheckTriggerUnderPlayer(int client)
{
	...
	#if defined USE_EXTENSION
	int ent = TraceTriggers(client, flStart, flEnd);

	#else

	DataPack pack = new DataPack();
	TR_EnumerateEntities(flStart, flEnd, PARTITION_TRIGGER_EDICTS, RayType_EndPoint, GetFirstTrigger, pack);
	pack.Reset();

	if(!pack.IsReadable(4))
	{
		delete pack;
		return 0;
	}
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
	if(IsValidClient(client) && g_bWantsStageRecords[client] && !IsFakeClient(client) && IsPlayerAlive(client))
	{
		...
		if(Velocity > 0)
		{
			int trigger = CheckTriggerUnderPlayer(client);
...
bool CheckIfTriggerAlreadyUsedInCurrentSequence(int client, int trigger)
{
	int iSize = GetArraySize(g_NormalPlayerTimesTriggerstoTriggerCurrentRunOnly[client]);

	//This is to prevent players from going backwards and getting new segments that are impossible...
	char IgnoreTriggerName[12];
	Format(IgnoreTriggerName, sizeof(IgnoreTriggerName), "-%i", trigger);
	
	for(int i = 0;i < iSize;i++)
	{
		char ArrayStringName[12];
	...
	char SequenceName[12], ArrayStringName[12];
...
void CheckTriggerSequence(int client, int trigger)
{
	int iSize = GetArraySize(g_NormalPlayerTimesTriggerstoTrigger[client]);
...
void PushClientRecordString(int client, char str[512], int r, int g, int b)
{
	//PrintToChat(client, "pushing %s", str);
	
	int iSize = GetArraySize(PlayerRecordStringEntries[client]);
...
void DisplayClientTimesToSpectator(int spectator, int client)
{
	int iSize = GetArraySize(PlayerRecordStringEntries[client]);
	if(iSize == 0)
		return;
	...
	for(int i = 0;i < iSize-1;i++)
	{
		...
		int r = GetArrayCell(PlayerRecordStringEntries[client], i, 64);
		int g = GetArrayCell(PlayerRecordStringEntries[client], i, 65);
		int b = GetArrayCell(PlayerRecordStringEntries[client], i, 66);
...
void DisplayClientPastTimes(int client)
{
	int iSize = GetArraySize(PlayerRecordStringEntries[client]);
	if(iSize == 0)
		return;
	...
	for(int i = 0;i < iSize-1;i++)
	{
		...
		int g = GetArrayCell(PlayerRecordStringEntries[client], i, 65);
		int b = GetArrayCell(PlayerRecordStringEntries[client], i, 66);
		int flash = GetArrayCell(PlayerRecordStringEntries[client], i, 67);
...
float GetPredictedNextTimeTriggerSequence(int client)
{
...
void ShowRecordOverlay(int client)
{
	...
	else
	{
		...
		//Get color degree
		float Percentage = (g_ClientCurrentTime[client] / PredictedNextTime) * 125.0;
	...
	char TempStringWithRandomTail[512];
...
void AddTimeFormatedFromSecondsToString(char str[512], float totalseconds, bool RandomizeEndingDecimal)
{
...
public void OnGameFrame()
{
	...
	for(int client = frames;client <= MAXPLAYERS;client+=10)
	{
...
void ShowHudMsg(int client, char[] message, float x, float y, int r, int g, int b, int a, int channel, bool flash, bool past = false)
{
	float holdtime = 0.11;
	if(past)
		if(iEngineVersion == Engine_CSS)
